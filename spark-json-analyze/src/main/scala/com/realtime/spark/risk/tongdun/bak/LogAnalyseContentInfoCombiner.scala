...
import com.realtime.spark.risk.tongdun.core.LogAnalyseRequestInfo
 ...
 * Date:     17/2/24.
 */
object LogAnalyseContentInfoCombiner {
        ...
        hitRulesMessage match {
          case Some(hitRulesList: List[Map[String, Object]]) => {
            for (hitRulesMap <- hitRulesList) {
              val hitRulesInfoJson = JsonMapUtils.mapToJson(hitRulesMap)
              hitRulesJsonInfoBuilder.append(extraFieldInfo + requestInfo + combine + hitRulesInfoJson + "\t" + timeInfo + "\n")
            }
          }
        }
